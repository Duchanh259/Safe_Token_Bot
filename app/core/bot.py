...
class TelegramBot:
    ...
    def _format_security_results(self, token_check_result: Dict[str, Any], language: str) -> str:
        ...
        # Sử dụng dịch từ translator thay vì chuỗi cứng
        result_message = f"🔍 {translator.get_text('contract_analysis_title', language)} 🔍\n\n"
        
        # Thêm thông tin token cơ bản
        result_message += f"* {translator.get_text('basic_info_title', language)}:\n"
        
        # Thêm tên token nếu có
        if token_info.get("name"):
            result_message += f"- {translator.get_text('token_name', language, name=token_info.get('name', 'N/A'))}\n"
        
        # Thêm ký hiệu token nếu có
        if token_info.get("symbol"):
            result_message += f"- {translator.get_text('token_symbol', language, symbol=token_info.get('symbol', 'N/A'))}\n"
        
        # Thêm thông tin chủ hợp đồng
        owner = token_info.get("owner", "N/A")
        result_message += f"- {translator.get_text('contract_owner', language, owner=owner)}\n"
        ...
        if token_info.get("total_supply") is not None:
            ...
            result_message += f"- {translator.get_text('token_total_supply', language, supply=formatted_supply)}\n"
        
        # Thêm thông tin về holders
        top_holders = holders_data.get("holders", [])
        result_message += f"- {translator.get_text('top_holders_list', language)}:\n"
        ...
        if top_holders and len(top_holders) > 0:
            ...
            result_message += f"  {translator.get_text('no_holder_data', language)}\n"
        
        # Thêm thông tin bể thanh khoản
        result_message += f"- {translator.get_text('liquidity_pool', language, status=translator.get_text('not_verified', language))}\n"
        ...
        if sell_tax == "N/A" or not sell_tax:
            sell_tax = "0"
        result_message += f"- {translator.get_text('token_taxes', language, buy=buy_tax, sell=sell_tax)}\n\n"
        ...
        if not is_verified:
            ...
            danger_items.append(f"⚠️ {translator.get_text('contract_not_verified', language)}")
        
        # Kiểm tra hàm mint
        has_mint = security.get("has_mint", False)
        if has_mint:
            dangerous_functions_found = True
            danger_items.append(f"⚠️ {translator.get_text('unlimited_mint', language)}")
        
        # Kiểm tra hàm blacklist
        has_blacklist = security.get("has_blacklist", False)
        if has_blacklist:
            dangerous_functions_found = True
            danger_items.append(f"⚠️ {translator.get_text('blacklist_function_warning', language)}")
        
        # Kiểm tra hàm self-destruct
        has_self_destruct = security.get("has_self_destruct", False)
        if has_self_destruct:
            dangerous_functions_found = True
            danger_items.append(f"⚠️ {translator.get_text('selfdestruct_function_warning', language)}")
        
        # Kiểm tra hàm pause
        has_pause = security.get("has_pause", False)
        if has_pause:
            dangerous_functions_found = True
            danger_items.append(f"⚠️ {translator.get_text('pause_function_warning', language)}")
        
        # Kiểm tra hàm revoke/revert transactions
        has_revoke = security.get("has_revoke", False)
        if has_revoke:
            dangerous_functions_found = True
            danger_items.append(f"⚠️ {translator.get_text('modify_balance_warning', language)}")
        
        # Thêm kiểm tra honeypot
        is_honeypot = security.get("is_honeypot", False)
        if is_honeypot:
            dangerous_functions_found = True
            danger_items.append(f"⚠️ {translator.get_text('honeypot_warning', language)}")
        
        # Hiển thị mục chức năng nguy hiểm
        result_message += f"* {translator.get_text('dangerous_functions_title', language)}:\n"
        ...
        if not is_verified or has_self_destruct or has_revoke:
            risk_level = translator.get_text('risk_level_critical', language)
            risk_desc = translator.get_text('risk_level_critical_desc', language)
        elif has_blacklist or has_pause or has_mint:
            risk_level = translator.get_text('risk_level_high', language)
            risk_desc = translator.get_text('risk_level_high_desc', language)
        else:
            risk_level = translator.get_text('risk_level_low', language)
            risk_desc = translator.get_text('risk_level_low_desc', language)
        
        # Thêm đánh giá BỎ định dạng HTML
        result_message += f"\n* {translator.get_text('contract_assessment', language)}: {risk_level}\n"
        result_message += f"{risk_desc}\n\n"
        
        # Thêm disclaimer
        result_message += f"{translator.get_text('disclaimer', language)}"
